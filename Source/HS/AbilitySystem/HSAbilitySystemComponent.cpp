...
void UHSAbilitySystemComponent::ProcessAbilityInput(float DeltaTime, bool bGamePaused)
{
	...
	for (const FGameplayAbilitySpecHandle& SpecHandle : InputHeldSpecHandles)
	{
		if (const FGameplayAbilitySpec* AbilitySpec = FindAbilitySpecFromHandle(SpecHandle))
		{
			...
			{
				const UHSGameplayAbility* AbilityCDO = CastChecked<UHSGameplayAbility>(AbilitySpec->Ability);

				if (AbilityCDO->GetActivationPolicy() == EHSAbilityActivationPolicy::WhileInputActive)
				{
		...
		if (FGameplayAbilitySpec* AbilitySpec = FindAbilitySpecFromHandle(SpecHandle))
		{
			if (AbilitySpec->Ability)
			{
				...
				else
				{
					...
					if (AbilityCDO->GetActivationPolicy() == EHSAbilityActivationPolicy::OnInputTriggered)
					{
						AbilitiesToActivate.AddUnique(AbilitySpec->Handle);
					}
				}
