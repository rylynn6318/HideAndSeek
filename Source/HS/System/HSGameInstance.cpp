...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HSGameInstance)

namespace HS
{
	static bool bTestEncryption = false;
	static FAutoConsoleVariableRef CVarHSTestEncryption(
		TEXT("HS.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarHSUseDTLSEncryption(
		TEXT("HS.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using HS.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarHSTestDTLSFingerprint(
		TEXT("HS.TestDTLSFingerprint"),
...
void UHSGameInstance::Init()
{
	...
	{
		ComponentManager->RegisterInitState(HSGameplayTags::InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(HSGameplayTags::InitState_DataAvailable, false, HSGameplayTags::InitState_Spawned);
		ComponentManager->RegisterInitState(HSGameplayTags::InitState_DataInitialized, false, HSGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(HSGameplayTags::InitState_GameplayReady, false, HSGameplayTags::InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UHSGameInstance::OnPreClientTravelToSession);
	}
}
...
AHSPlayerController* UHSGameInstance::GetPrimaryPlayerController() const
{
	return Cast<AHSPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool UHSGameInstance::CanJoinRequestedSession() const
{
...
void UHSGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error, ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UHSLocalPlayer* LocalPlayer = Cast<UHSLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
...
void UHSGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (HS::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (HS::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("HSTest.pem");
...
void UHSGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UHSGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (HS::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (HS::bUseDTLSEncryption)
		{
