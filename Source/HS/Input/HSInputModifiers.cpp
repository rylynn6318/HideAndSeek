...
#include "HSInputModifiers.h"

#include "EnhancedPlayerInput.h"
#include "GameFramework/PlayerController.h"
#include "Input/HSAimSensitivityData.h"
#include "Player/HSLocalPlayer.h"
#include "Settings/HSSettingsShared.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(HSInputModifiers)

DEFINE_LOG_CATEGORY_STATIC(LogHSInputModifiers, Log, All);

//////////////////////////////////////////////////////////////////////
// HSInputModifiersHelpers

namespace HSInputModifiersHelpers
{
	/** Returns the owning HSLocalPlayer of an Enhanced Player Input pointer */
	static UHSLocalPlayer* GetLocalPlayer(const UEnhancedPlayerInput* PlayerInput)
	{
		if (PlayerInput)
		{
			if (APlayerController* PC = Cast<APlayerController>(PlayerInput->GetOuter()))
			{
				return Cast<UHSLocalPlayer>(PC->GetLocalPlayer());
			}
		}
		return nullptr;
	}
...
//////////////////////////////////////////////////////////////////////
// UHSSettingBasedScalar

FInputActionValue UHSSettingBasedScalar::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	{
		if (UHSLocalPlayer* LocalPlayer = HSInputModifiersHelpers::GetLocalPlayer(PlayerInput))
		{
			const UClass* SettingsClass = UHSSettingsShared::StaticClass();
			UHSSettingsShared* SharedSettings = LocalPlayer->GetSharedSettings();
...
//////////////////////////////////////////////////////////////////////
// UHSInputModifierDeadZone

FInputActionValue UHSInputModifierDeadZone::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	EInputActionValueType ValueType = CurrentValue.GetValueType();
	UHSLocalPlayer* LocalPlayer = HSInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (ValueType == EInputActionValueType::Boolean || !LocalPlayer)
	{
		return CurrentValue;
	}
	
	UHSSettingsShared* Settings = LocalPlayer->GetSharedSettings();
	ensure(Settings);
...
FLinearColor UHSInputModifierDeadZone::GetVisualizationColor_Implementation(FInputActionValue SampleValue, FInputActionValue FinalValue) const
{
...
//////////////////////////////////////////////////////////////////////
// UHSInputModifierGamepadSensitivity

FInputActionValue UHSInputModifierGamepadSensitivity::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	const EHSGamepadSensitivity Sensitivity = (TargetingType == EHSTargetingType::Normal) ? Settings->GetGamepadLookSensitivityPreset() : Settings->GetGamepadTargetingSensitivityPreset();
...
//////////////////////////////////////////////////////////////////////
// UHSInputModifierAimInversion

FInputActionValue UHSInputModifierAimInversion::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	UHSLocalPlayer* LocalPlayer = HSInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (!LocalPlayer)
	{
