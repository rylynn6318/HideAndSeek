...
#include "HSInputModifiers.generated.h"
...
class UHSSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the HS Shared game settings. 
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "HS Settings Driven Dead Zone"))
class UHSInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class EHSTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in HS Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "HS Gamepad Sensitivity"))
class UHSInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	TObjectPtr<const UHSAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the HS Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "HS Aim Inversion Setting"))
class UHSInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
