...
#include "HSAudioSettings.generated.h"
...
USTRUCT()
struct HS_API FHSSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "HSAudioSettings"))
class HS_API UHSAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FHSSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FHSSubmixEffectChainMap> LDRAudioSubmixEffectChain;
};
