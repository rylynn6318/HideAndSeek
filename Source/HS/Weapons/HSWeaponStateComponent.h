...
#include "HSWeaponStateComponent.generated.h"
...
struct FHSScreenSpaceHitLocation
{
...
struct FHSServerSideHitMarkerBatch
{
	FHSServerSideHitMarkerBatch() { }

	FHSServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FHSScreenSpaceHitLocation> Markers;
...
UCLASS()
class UHSWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UHSWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FHSScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FHSScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FHSServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
