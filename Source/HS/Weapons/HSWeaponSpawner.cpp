...
#include "HSWeaponSpawner.h"

#include "AbilitySystemBlueprintLibrary.h"
#include "Components/CapsuleComponent.h"
#include "Components/StaticMeshComponent.h"
#include "Engine/World.h"
#include "Equipment/HSPickupDefinition.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HSWeaponSpawner)
...
// Sets default values
AHSWeaponSpawner::AHSWeaponSpawner()
{
	...
	CollisionVolume->OnComponentBeginOverlap.AddDynamic(this, &AHSWeaponSpawner::OnOverlapBegin);
...
void AHSWeaponSpawner::BeginPlay()
{
	Super::BeginPlay();
	...
	else if (const UWorld* World = GetWorld())
	{
		...
		{
			UE_LOG(LogHS, Error, TEXT("'%s' does not have a valid weapon definition! Make sure to set this data on the instance!"), *GetNameSafe(this));	
		}
	}
}

void AHSWeaponSpawner::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
// Called every frame
void AHSWeaponSpawner::Tick(float DeltaTime)
{
...
void AHSWeaponSpawner::OnConstruction(const FTransform& Transform)
{
...
void AHSWeaponSpawner::OnOverlapBegin(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepHitResult)
{
...
void AHSWeaponSpawner::CheckForExistingOverlaps()
{
...
void AHSWeaponSpawner::AttemptPickUpWeapon_Implementation(APawn* Pawn)
{
	if (GetLocalRole() == ROLE_Authority && bIsWeaponAvailable && UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(Pawn))
	{
		TSubclassOf<UHSInventoryItemDefinition> WeaponItemDefinition = WeaponDefinition ? WeaponDefinition->InventoryItemDefinition : nullptr;
...
void AHSWeaponSpawner::StartCoolDown()
{
	if (UWorld* World = GetWorld())
	{
		World->GetTimerManager().SetTimer(CoolDownTimerHandle, this, &AHSWeaponSpawner::OnCoolDownTimerComplete, CoolDownTime);
	}
}

void AHSWeaponSpawner::ResetCoolDown()
{
	UWorld* World = GetWorld();
	...
	if (GetLocalRole() == ROLE_Authority)
	{
		...
		if (World)
		{
			World->GetTimerManager().SetTimer(CheckOverlapsDelayTimerHandle, this, &AHSWeaponSpawner::CheckForExistingOverlaps, CheckExistingOverlapDelay);
		}
	}
...
void AHSWeaponSpawner::OnCoolDownTimerComplete()
{
	ResetCoolDown();
}

void AHSWeaponSpawner::SetWeaponPickupVisibility(bool bShouldBeVisible)
{
	WeaponMesh->SetVisibility(bShouldBeVisible, true);
}

void AHSWeaponSpawner::PlayPickupEffects_Implementation()
{
...
void AHSWeaponSpawner::PlayRespawnEffects_Implementation()
{
	if (WeaponDefinition != nullptr)
	{
...
void AHSWeaponSpawner::OnRep_WeaponAvailability()
{
...
void AHSWeaponSpawner::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(AHSWeaponSpawner, bIsWeaponAvailable);
}

int32 AHSWeaponSpawner::GetDefaultStatFromItemDef(const TSubclassOf<UHSInventoryItemDefinition> WeaponItemClass, FGameplayTag StatTag)
{
	if (WeaponItemClass != nullptr)
	{
		if (UHSInventoryItemDefinition* WeaponItemCDO = WeaponItemClass->GetDefaultObject<UHSInventoryItemDefinition>())
		{
