...
#include "Equipment/HSGameplayAbility_FromEquipment.h"

#include "HSGameplayAbility_RangedWeapon.generated.h"

enum ECollisionChannel : int;

class APawn;
class UHSRangedWeaponInstance;
class UObject;
...
UENUM(BlueprintType)
enum class EHSAbilityTargetingSource : uint8
{
...
/**
 * UHSGameplayAbility_RangedWeapon
 *
 * An ability granted by and associated with a ranged weapon instance
 */
UCLASS()
class UHSGameplayAbility_RangedWeapon : public UHSGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	UHSGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="HS|Ability")
	UHSRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		UHSRangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, EHSAbilityTargetingSource Source) const;
