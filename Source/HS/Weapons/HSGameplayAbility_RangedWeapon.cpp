...
#include "HSGameplayAbility_RangedWeapon.h"
#include "Weapons/HSRangedWeaponInstance.h"
#include "Physics/HSCollisionChannels.h"
#include "HSLogChannels.h"
#include "AIController.h"
#include "NativeGameplayTags.h"
#include "Weapons/HSWeaponStateComponent.h"
#include "AbilitySystemComponent.h"
#include "AbilitySystem/HSGameplayAbilityTargetData_SingleTargetHit.h"
#include "DrawDebugHelpers.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(HSGameplayAbility_RangedWeapon)
...
namespace HSConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("HS.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("HS.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("HS.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
UHSGameplayAbility_RangedWeapon::UHSGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UHSRangedWeaponInstance* UHSGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UHSRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UHSGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogHSAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UHSRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UHSGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UHSGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UHSGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return HS_TraceChannel_Weapon;
}

FHitResult UHSGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UHSGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UHSGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EHSAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UHSWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UHSWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EHSAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EHSAbilityTargetingSource::CameraTowardsFocus) || (Source == EHSAbilityTargetingSource::PawnTowardsFocus) || (Source == EHSAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EHSAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EHSAbilityTargetingSource::WeaponForward) || (Source == EHSAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EHSAbilityTargetingSource::PawnTowardsFocus) || (Source == EHSAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UHSGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (HSConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, HSConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UHSGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UHSRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EHSAbilityTargetingSource::CameraTowardsFocus);
		...
		if (HSConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, HSConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UHSGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UHSRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (HSConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, HSConsoleVariables::DrawBulletHitRadius, FColor::Red, false, HSConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UHSGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UHSGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UHSGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UHSWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UHSWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogHSAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UHSGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UHSWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UHSWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FHSGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FHSGameplayAbilityTargetData_SingleTargetHit();
