...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HSWeaponInstance)

class UAnimInstance;
struct FGameplayTagContainer;

UHSWeaponInstance::UHSWeaponInstance(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	if (APawn* Pawn = GetPawn())
	{
		...
		{
			if (UHSHealthComponent* HealthComponent = UHSHealthComponent::FindHealthComponent(GetPawn()))
			{
...
void UHSWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UHSWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
...
void UHSWeaponInstance::UpdateFiringTime()
{
	UWorld* World = GetWorld();
	check(World);
...
float UHSWeaponInstance::GetTimeSinceLastInteractedWith() const
{
	UWorld* World = GetWorld();
	check(World);
...
TSubclassOf<UAnimInstance> UHSWeaponInstance::PickBestAnimLayer(bool bEquipped, const FGameplayTagContainer& CosmeticTags) const
{
	const FHSAnimLayerSelectionSet& SetToQuery = (bEquipped ? EquippedAnimSet : UneuippedAnimSet);
	return SetToQuery.SelectBestLayer(CosmeticTags);
}

const FPlatformUserId UHSWeaponInstance::GetOwningUserId() const
{
...
void UHSWeaponInstance::ApplyDeviceProperties()
{
...
void UHSWeaponInstance::RemoveDeviceProperties()
{
	const FPlatformUserId UserId = GetOwningUserId();
...
void UHSWeaponInstance::OnDeathStarted(AActor* OwningActor)
{
