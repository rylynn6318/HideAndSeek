...
#include "HSRangedWeaponInstance.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(HSRangedWeaponInstance)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_HS_Weapon_SteadyAimingCamera, "HS.Weapon.SteadyAimingCamera");

UHSRangedWeaponInstance::UHSRangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UHSRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UHSRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UHSRangedWeaponInstance::UpdateDebugVisualization()
{
...
void UHSRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UHSRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UHSRangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UHSRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UHSRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UHSRangedWeaponInstance::AddSpread()
{
...
float UHSRangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float UHSRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool UHSRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UHSRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const UHSCameraComponent* CameraComponent = UHSCameraComponent::FindCameraComponent(Pawn))
	{
		...
		AimingAlpha = (TopCameraTag == TAG_HS_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
