...
#include "HSSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class EHSAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
class UHSLocalPlayer;

/**
 * UHSSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UHSSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UHSSettingsShared, FOnSettingChangedEvent, UHSSettingsShared* Settings);
...
public:
	...
	static UHSSettingsShared* CreateTemporarySettings(const UHSLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UHSSettingsShared* LoadOrCreateSettings(const UHSLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UHSSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UHSLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	EHSAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(EHSAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	EHSAllowBackgroundAudioSetting AllowAudioInBackground = EHSAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	EHSGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(EHSGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	EHSGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(EHSGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	EHSGamepadSensitivity GamepadLookSensitivityPreset = EHSGamepadSensitivity::Normal;
	UPROPERTY()
	EHSGamepadSensitivity GamepadTargetingSensitivityPreset = EHSGamepadSensitivity::Normal;
