...
#include "Performance/HSPerformanceSettings.h"
#include "DeviceProfiles/DeviceProfileManager.h"
#include "DeviceProfiles/DeviceProfile.h"
#include "HAL/PlatformFramePacer.h"
#include "Development/HSPlatformEmulationSettings.h"
#include "SoundControlBus.h"
#include "AudioModulationStatics.h"
#include "Audio/HSAudioSettings.h"
#include "Audio/HSAudioMixEffectsSubsystem.h"
#include "EnhancedActionKeyMapping.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(HSSettingsLocal)
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("HS.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("HS.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("HS.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("HS.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("HS.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("HS.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("HS.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("HS.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("HS.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("HS.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FHSScalabilitySnapshot::FHSScalabilitySnapshot()
{
...
namespace HSSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FHSScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	{
		const UHSPlatformSpecificRenderingSettings* PlatformSettings = UHSPlatformSpecificRenderingSettings::Get();
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UHSSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UHSSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UHSSettingsLocal::UHSSettingsLocal()
{
...
void UHSSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
...
void UHSSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UHSPlatformSpecificRenderingSettings* PlatformSettings = UHSPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EHSFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	HSSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UHSSettingsLocal::ResetToCurrentSettings()
{
...
void UHSSettingsLocal::BeginDestroy()
{
...
UHSSettingsLocal* UHSSettingsLocal::Get()
{
	return GEngine ? CastChecked<UHSSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UHSSettingsLocal::ConfirmVideoMode()
{
...
float UHSSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UHSPlatformSpecificRenderingSettings* PlatformSettings = UHSPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == EHSFramePacingMode::DesktopStyle)
	{
...
int32 UHSSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return HSSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UHSSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FHSScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UHSSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UHSSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UHSSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UHSSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UHSSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UHSSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
EHSStatDisplayMode UHSSettingsLocal::GetPerfStatDisplayState(EHSDisplayablePerformanceStat Stat) const
{
	if (const EHSStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return EHSStatDisplayMode::Hidden;
	}
}

void UHSSettingsLocal::SetPerfStatDisplayState(EHSDisplayablePerformanceStat Stat, EHSStatDisplayMode DisplayMode)
{
	if (DisplayMode == EHSStatDisplayMode::Hidden)
	{
...
float UHSSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UHSSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UHSSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UHSSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UHSSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UHSSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UHSSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UHSSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UHSSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UHSSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UHSSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UHSSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UHSSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UHSSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UHSSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UHSSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return HSSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UHSSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return HSSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UHSSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UHSSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UHSPlatformSpecificRenderingSettings* PlatformSettings = UHSPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == EHSFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return HSSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UHSSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UHSPlatformSpecificRenderingSettings* PlatformSettings = UHSPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EHSFramePacingMode::MobileStyle)
	{
...
void UHSSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UHSSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UHSPlatformSpecificRenderingSettings* PlatformSettings = UHSPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EHSFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = HSSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UHSSettingsLocal::ClampMobileQuality()
{
	const UHSPlatformSpecificRenderingSettings* PlatformSettings = UHSPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == EHSFramePacingMode::MobileStyle)
	{
...
void UHSSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = HSSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UHSSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = HSSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = HSSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UHSSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UHSSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UHSSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UHSSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UHSSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = HSSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UHSSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UHSSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (UHSAudioMixEffectsSubsystem* HSAudioMixEffectsSubsystem = World->GetSubsystem<UHSAudioMixEffectsSubsystem>())
			{
				HSAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool UHSSettingsLocal::CanRunAutoBenchmark() const
{
	const UHSPlatformSpecificRenderingSettings* PlatformSettings = UHSPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UHSSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UHSSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UHSSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UHSSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UHSSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UHSSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UHSSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UHSSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UHSSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UHSSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UHSSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UHSSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UHSSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UHSSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UHSSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UHSSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UHSSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UHSSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UHSSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == EHSFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = HSSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UHSSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UHSSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UHSSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UHSAudioSettings* HSAudioSettings = GetDefault<UHSAudioSettings>())
			{
				...
				if (UObject* ObjPath = HSAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from HS Audio Settings."));
					}
				}

				if (UObject* ObjPath = HSAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from HS Audio Settings."));
					}
				}

				if (UObject* ObjPath = HSAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from HS Audio Settings."));
					}
				}

				if (UObject* ObjPath = HSAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from HS Audio Settings."));
					}
				}

				if (UObject* ObjPath = HSAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from HS Audio Settings."));
					}
				}

				if (UObject* ObjPath = HSAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from HS Audio Settings."));
					}
				}
			}
		}
...
void UHSSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UHSSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UHSPlatformSpecificRenderingSettings* PlatformSettings = UHSPlatformSpecificRenderingSettings::Get();
	const TArray<FHSQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FHSQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UHSPlatformEmulationSettings* Settings = GetDefault<UHSPlatformEmulationSettings>();
	...
	{
	case EHSFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case EHSFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case EHSFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UHSSettingsLocal::UpdateConsoleFramePacing()
{
...
void UHSSettingsLocal::UpdateDesktopFramePacing()
{
...
void UHSSettingsLocal::UpdateMobileFramePacing()
{
...
void UHSSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
