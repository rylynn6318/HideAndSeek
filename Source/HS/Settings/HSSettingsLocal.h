...
#include "HSSettingsLocal.generated.h"

enum class ECommonInputType : uint8;
enum class EHSDisplayablePerformanceStat : uint8;
enum class EHSStatDisplayMode : uint8;
...
USTRUCT()
struct FHSScalabilitySnapshot
{
	GENERATED_BODY()

	FHSScalabilitySnapshot();
 ...
 */
UCLASS()
class UHSSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	EHSStatDisplayMode GetPerfStatDisplayState(EHSDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(EHSDisplayablePerformanceStat Stat, EHSStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UHSSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<EHSDisplayablePerformanceStat, EHSStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FHSScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FHSScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UHSSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
