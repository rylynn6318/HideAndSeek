...
#include "HSCameraMode_ThirdPerson.h"
#include "Camera/HSCameraMode.h"
#include "Components/PrimitiveComponent.h"
#include "Camera/HSPenetrationAvoidanceFeeler.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "HSCameraAssistInterface.h"
#include "GameFramework/Controller.h"
#include "GameFramework/Character.h"
#include "Math/RotationMatrix.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(HSCameraMode_ThirdPerson)

namespace HSCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UHSCameraMode_ThirdPerson::UHSCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FHSPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FHSPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FHSPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FHSPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FHSPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FHSPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FHSPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UHSCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UHSCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UHSCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UHSCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IHSCameraAssistInterface* TargetControllerAssist = Cast<IHSCameraAssistInterface>(TargetController);

	IHSCameraAssistInterface* TargetActorAssist = Cast<IHSCameraAssistInterface>(TargetActor);
	...
	IHSCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IHSCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IHSCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IHSCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UHSCameraMode_ThirdPerson::PreventCameraPenetration(class AActor const& ViewTarget, FVector const& SafeLoc, FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
#if ENABLE_DRAW_DEBUG
	...
	//TODO IHSCameraTarget.GetIgnoredActorsForCameraPentration();
	...
	{
		FHSPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(HSCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UHSCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UHSCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
