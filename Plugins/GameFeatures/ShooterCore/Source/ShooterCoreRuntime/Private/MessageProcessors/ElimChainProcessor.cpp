...
void UElimChainProcessor::StartListening()
{
	...
	AddListenerHandle(MessageSubsystem.RegisterListener(ElimChain::TAG_HS_Elimination_Message, this, &ThisClass::OnEliminationMessage));
}

void UElimChainProcessor::OnEliminationMessage(FGameplayTag Channel, const FHSVerbMessage& Payload)
{
	...
	if (Payload.Instigator != Payload.Target)
	{
		if (APlayerState* InstigatorPS = Cast<APlayerState>(Payload.Instigator))
		{
			...
			else
			{
				...
				{
					FHSVerbMessage ElimChainMessage;
